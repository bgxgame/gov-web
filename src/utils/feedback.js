...
/**
 * 提供统一的成功、警告、错误提示与确认弹框。
 * 存在原因：收敛页面内零散提示逻辑，统一中文文案、交互行为和弹层体验。
 * 输入输出：输入为消息内容或异常对象，输出为统一的 Element Plus 消息行为。
 * 关联链路：request 拦截器、页面表单提交、删除确认、审批确认等动作反馈。
 */
const MESSAGE_MAP = {
  Error: '系统繁忙，请稍后重试',
...
/**
 * 统一处理页面动作异常；用户取消时静默忽略，其余异常只在未提示过时补充兜底提示。
 */
